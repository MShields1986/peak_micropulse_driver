...
void PeakHandler::sendReset() {
    ...
    while (attempts <= max_attempts and not success) {
        ...
        //sendCommand("RST 50");
        sendCommand("RST 100");
        //sendCommand("RST " + std::to_string(frequency_));
        ...
        if ((int)response.front() == 35) {
            ...
            //logToConsole("System Type " + std::to_string((int)response[5]));
            // Decode of System type byte is as follows:
                                                    ...
                                                    //     4 = MPLT
                                                    //     5 = MicroPulse 6
            // TODO: These don't seem to be read correctly...
            logToConsole("Default data output format (DOF) " + std::to_string((uint8_t)response[11]));
            logToConsole("Actual data output format (DOF) " + std::to_string((uint8_t)response[8]));
            logToConsole("Default digitisation rate " + std::to_string((uint8_t)response[9]) + " MHz");
            logToConsole("Actual digitisation rate " + std::to_string((uint8_t)response[10]) + " MHz");
            logToConsole(" -------- ---------------- --------");
